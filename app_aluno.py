...
def init_supabase() -> Client:
    if not SUPABASE_URL or not SUPABASE_KEY:
        st.error("Chaves do Supabase não encontradas no arquivo .env!")
        st.stop()
...
def carregar_respostas_aluno(matricula: str):
    ...
    try:
        res = supabase.table("respostas_alunos") \
            ...
            .execute()
        if res.data:
            return set(item["habilidade_id"] for item in res.data)
    except Exception:
        pass
    return set()
...
# =====================================================================
    ...
    habs_tema = list(dict_questoes.keys())
        ...
        with st.container(border=True):
            ...
            if not dict_questoes:
                ...
                hab_escolhida = st.selectbox(
                    ...
                    options=habs_tema,
                    format_func=lambda x: f"{'✅' if x in st.session_state.consolidadas else '📌'} {x} - {dict_habs[x]['titulo']}"
                )
        ...
        if st.session_state.resposta_enviada:
            ...
            if escolha_feita in opcoes:
                ...
                if is_correto:
                    ...
                    st.info(f"💡 **Explicação:** {q['explicacao_correta']}")
                else:
