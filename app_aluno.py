...
# 1. Carregamento de variáveis de ambiente de forma segura
load_dotenv()
...
if not SUPABASE_URL or not SUPABASE_KEY:
    st.error("⚠️ Credenciais do Supabase não encontradas! Verifique o arquivo .env ou Secrets no Streamlit Cloud.")
    st.stop()
...
# 2. Funções de Autenticação e Busca no Supabase
...
def carregar_respostas_aluno(matricula: str):
    """Busca as habilidades que o aluno já acertou previamente."""
    try:
...
def carregar_habilidades_por_unidade(unidade: str):
    """Busca habilidades e questões de uma Unidade Temática."""
    try:
...
# =====================================================================
# TELA 4: EXECUÇÃO DOS EXERCÍCIOS
# =====================================================================
    ...
    if total_q > 0 and idx < total_q:
        ...
        # Gerenciamento de estado do Radio
        radio_key = f"radio_q_{q['id']}_{idx}"
        ...
        if st.session_state.resposta_enviada:
            escolha_feita = st.session_state.escolha_atual or escolha
