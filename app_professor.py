...
# =====================================================================
# AUTENTICAÇÃO DO PROFESSOR (ESTADO DE SESSÃO E LOGIN)
# =====================================================================
...
if "prof_email" not in st.session_state:
    st.session_state.prof_email = ""

def autenticar_professor(email: str, senha: str):
    """Valida o e-mail e senha do docente na tabela 'professores'."""
    try:
        res = supabase.table("professores") \
        ...
        if res.data:
            ...
            return False, "E-mail ou senha de professor incorretos."
    except Exception as e:
        return False, f"Erro de conexão com o banco de dados: {e}"


# Se o professor não estiver logado, exibe a tela de login e interrompe o restante
if not st.session_state.prof_autenticado:
    st.markdown("# 👨‍🏫 Portal do Docente | GPS-Math")
    st.markdown("##### Área de acesso restrito a professores e gestão pedagógica.")
    st.divider()

    col_login, _ = st.columns([1, 1])

    with col_login:
        with st.form("form_login_professor"):
            st.subheader("🔒 Acesso Restrito ao Painel")
            email_prof = st.text_input("E-mail institucional:", placeholder="professor@escola.com.br")
            senha_prof = st.text_input("Senha:", type="password", placeholder="Digite sua senha")
            
            btn_login_prof = st.form_submit_button("🚀 Entrar no Painel", type="primary")

            if btn_login_prof:
                if not email_prof.strip() or not senha_prof.strip():
                    st.error("Por favor, preencha o e-mail e a senha.")
                else:
                    sucesso, dados = autenticar_professor(email_prof, senha_prof)
                    if sucesso:
                        st.session_state.prof_autenticado = True
                        st.session_state.prof_nome = dados.get("nome", "Professor")
                        st.session_state.prof_email = dados.get("email", email_prof)
                        st.success("Login efetuado com sucesso!")
                        st.rerun()
                    else:
                        st.error(dados)

    st.stop()  # Impede o carregamento dos dados e do painel antes do login


# =====================================================================
# 2. CARREGAMENTO E TRATAMENTO DE DADOS (APÓS AUTENTICAÇÃO)
# =====================================================================
...
# Barra Superior de Identificação do Professor e Logout
...
with col_user:
    ...
    st.caption(f"{st.session_state.prof_email}")
    if st.button("🚪 Sair do Painel", type="secondary"):
        st.session_state.prof_autenticado = False
        st.session_state.prof_nome = ""
        st.session_state.prof_email = ""
        st.rerun()
...
# =====================================================================
tab_turma, tab_alunos, tab_moderacao, tab_banco = st.tabs([
...
# ---------------------------------------------------------------------
# ABA 3: MODERAÇÃO E CORREÇÃO DE GABARITO
# ---------------------------------------------------------------------
with tab_moderacao:
    ...
    if df_questoes.empty:
        ...
        if metodo_busca == "ID da Questão":
            ...
            with col_busca2:
                ...
                opcoes_q = {
                    row["id"]: f"ID {row['id']} - {str(row['enunciado'])[:60]}..." 
        ...
        if questao_alvo:
            ...
            with st.form("form_edicao_questao"):
                ...
                with c_dif:
                    ...
                    idx_dif = 0 if dificuldade_atual == "Facil" else (1 if dificuldade_atual == "Media" else 2)
                    nova_dificuldade = st.selectbox("Nível de Dificuldade:", ["Facil", "Media", "Dificil"], index=idx_dif)
                ...
                if btn_salvar:
                    try:
                        ...
                        # 2. Recalcula o histórico de respostas dos alunos
